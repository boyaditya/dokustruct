...
class _LayoutResList(list):
    """A list subclass that can carry a `_stage_timings` attribute.

    `_run_layout_detection` builds the result via list comprehensions, which
    yields a plain `list` — plain lists cannot hold attributes, so attaching
    `_stage_timings` raised AttributeError. Wrapping in this subclass restores
    per-stage timing propagation for the Python benchmark.
    """
...
class BatchAnalyze:
    ...
    def __call__(
        ...
        # Wrap so we can attach `_stage_timings` later (plain list cannot hold attrs).
        if not isinstance(images_layout_res, _LayoutResList):
            images_layout_res = _LayoutResList(images_layout_res)
        layout_time = time.time() - t_layout
        # 2. 收集各类型检测区域
        ...
        if self.formula_enable:
            ...
            formula_time = time.time() - t_formula
        # 4. OCR 检测 (det) — 根据模式选择不同的处理方式
        t_ocr = time.time()
        ...
        # Attach per-stage timings to the result list so callers can extract them.
        # Values are in seconds (float). Keys match the JS pipeline stage names.
        # OCR is split into det (ocr_det) and rec (ocr_rec); both are inference.
        images_layout_res._stage_timings = {
            "layout": round(layout_time, 4),
            "ocr_det": round(ocr_det_time, 4),
            "ocr_rec": round(ocr_rec_time, 4),
            "formula": round(formula_time, 4),
            "table": round(table_time, 4),
        }
