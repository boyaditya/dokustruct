...
class _StageTimingList(list):
    """A list subclass that can carry a `_stage_timings` attribute, so the
    accumulated per-stage timings survive the infer_results rebuild and can be
    read by the benchmark runner (demo_batch.py)."""
    _stage_timings = None
...
def doc_analyze(
    ...
    _acc_stage_timings: Dict[str, float] = {}
    for index, batch_image in enumerate(batch_images):
        ...
        # Preserve per-stage timings (lost by results.extend, which copies elems).
        _bt = getattr(batch_results, "_stage_timings", None)
        if _bt:
            for _k, _v in _bt.items():
                _acc_stage_timings[_k] = _acc_stage_timings.get(_k, 0.0) + float(_v or 0.0)
    ...
    # Re-attach accumulated per-stage timings so downstream timing extraction
    # (demo_batch.py) can read model_list._stage_timings. Plain lists cannot
    # hold attributes, so wrap each PDF's page list in _StageTimingList.
    if _acc_stage_timings:
        rounded = {k: round(v, 4) for k, v in _acc_stage_timings.items()}
        wrapped = []
        for pages in infer_results:
            w = _StageTimingList(pages)
            w._stage_timings = dict(rounded)
            wrapped.append(w)
        infer_results = wrapped
