...
/**
 * Split line-level OCR boxes into word-level boxes by detecting large gaps.
 * Estimates word positions based on character spacing.
 * 
 * @param {Array} ocrResults - OCR results [[box, text, score], ...]
 * @returns {Array} Split OCR results with word-level boxes
 */
export function splitOcrBoxesByGaps(ocrResults) {
  const splitResults = [];
  
  // Helper to convert polygon to bbox
  const box42PolyToBox41 = (p) => {
    if (Array.isArray(p[0])) {
      // Already polygon format [[x,y], ...]
      return [p[0][0], p[0][1], p[2][0], p[2][1]];
    }
    // Already bbox format [x1,y1,x2,y2]
    return p;
  };
  
  for (const [box, text, score] of ocrResults) {
    if (!text || text.trim().length === 0) {
      splitResults.push([box, text, score]);
      continue;
    }
    
    // Split text by multiple spaces or large gaps
    const words = text.split(/\s{2,}|\s+/).filter(w => w.trim().length > 0);
    
    if (words.length <= 1) {
      // Single word or no split - keep original
      splitResults.push([box, text, score]);
      continue;
    }
    
    // Estimate word positions based on character count
    const bbox = box42PolyToBox41(box);
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      // Find word position in original text (handle multiple spaces)
      const wordStart = text.indexOf(word, charPos);
      const wordEnd = wordStart + wordChars;
      
      // Estimate bounding box for this word
      const startRatio = wordStart / totalChars;
      const endRatio = wordEnd / totalChars;
      
      const wordXMin = bbox[0] + startRatio * boxWidth;
      const wordXMax = bbox[0] + endRatio * boxWidth;
      ...
      splitResults.push([wordBox, word, score]);
      charPos = wordEnd;
    }
  }
